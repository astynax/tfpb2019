...
явно писать квантор общности `forall` — иначе я не смог бы написать просто
...
в ней тип `f` уже применён. Эта сигнатура делает явным кучу вещей:
...
ведь и `a` и `b` будут известны только при вызове `fmap` и значения `b` взять
неоткуда.
